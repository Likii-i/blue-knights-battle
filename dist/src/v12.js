// v1.2: v1.1 + deliberative look-backs (information gathering), open-area escape targeting,
// and lightweight spatial learning (safe/trap spots).
...
function clearance(world, x, y) {
  // Distance to the nearest wall (bigger => more open).
...
function setThought(world, agent, msg) {
  agent.thought.last = msg;
  agent.thought.since = world.time;
}

function upsertSpot(list, spot, now, mergeDist) {
  // Merge into nearest existing entry, keep list small.
  const md = mergeDist ?? 70;
  let bestI = -1;
  let bestD = Infinity;
...
function makeAgent(world) {
  const a = {
    ...
    // Debug "thoughts" (why it did something) to iterate on believability.
    thought: {
      last: "",
      since: 0,
    },

    // Tiny spatial memory (learned safe/trap spots, used by escape targeting).
    spatial: {
      safeSpots: [], // {x,y,score,lastAt}
      trapSpots: [], // {x,y,score,lastAt}
    },
...
// --- v1.1/v1.2: intent + planning + mistakes --------------------------------
...
function endIntent(world, agent) {
  ...
  // Spatial learning: where was it easier/harder to break pursuit?
  if (agent.intent.id === "ESCAPE") {
    const esc = agent.intent.escape;
    const avgThreat = esc.threatSamples > 0 ? esc.threatSum / esc.threatSamples : 0;
    const hereC = clearance(world, agent.x, agent.y);

    const safeNow = agent.sensors.threat.prox < 0.12 && agent.arousal.pursuitConfidence < 0.35;
    if (safeNow && esc.threatSamples > 0) {
      const score = hereC * (1 - clamp01(avgThreat));
      if (score > 35) {
        upsertSpot(agent.spatial.safeSpots, { x: agent.x, y: agent.y, score }, now, 85);
      }
    }

    // Reset per-escape stats.
    esc.threatSum = 0;
    esc.threatSamples = 0;
    esc.target = null;
    esc.until = 0;
...
function enterIntent(world, agent, id) {
  ...
  if (id === "EXPLORE") {
    ...
    const needOpen = agent.sensors.threat.aware || agent.arousal.pursuitConfidence > 0.3;
    const minC = needOpen ? 80 : 0;
    let wp = null;
    for (let i = 0; i < 10; i++) {
      const cand = {
      ...
      if (clearance(world, cand.x, cand.y) >= minC) {
        wp = cand;
        break;
      }
    }
    agent.intent.waypoint = wp ?? { x: randRange(agent.r, world.width - agent.r), y: randRange(agent.r, world.height - agent.r) };
  } else if (id === "FORAGE") {
    ...
    agent.intent.waypoint = null;
    agent.gaze.glance.activeUntil = 0;
  } else {
    agent.intent.sub = "RUN";
    agent.intent.waypoint = null;
    if (id === "ESCAPE") {
      agent.intent.sub = "RUN_TO_OPEN";
      agent.intent.escape.target = null;
      agent.intent.escape.until = 0;
      agent.intent.escape.threatSum = 0;
      agent.intent.escape.threatSamples = 0;
      agent.intent.escape.lastTrapAt = -Infinity;
    }
  }
...
function threatAngleForGlance(agent, moveAngle) {
  // Angle *toward* the threat (so if we face it, it can enter the FOV wedge).
  if (agent.belief.threat.quality > 0.2) {
    return angleTo(agent.belief.threat.x - agent.x, agent.belief.threat.y - agent.y);
  }
  if (agent.memory.threatPos) {
    return angleTo(agent.memory.threatPos.x - agent.x, agent.memory.threatPos.y - agent.y);
  }
  return wrapAngle((Number.isFinite(moveAngle) ? moveAngle : agent.heading) + Math.PI);
}

function updateGlanceDecision(world, agent, dt, moveAngle, clearanceHere, sprintT) {
  // A deliberate "should I look back now?" decision with cost/benefit tradeoff.
  const now = world.time;
  const g = agent.gaze.glance;
  const s = agent.sensors;

  const canConsider =
    agent.intent.id === "ESCAPE" ||
    (agent.intent.id === "FORAGE" &&
      s.threat.aware &&
      (s.uncertainty > 0.35 || agent.arousal.pursuitConfidence > 0.25));

  // If not relevant, decay urge and end any active glance.
  if (!canConsider) {
    g.urge = lerp(g.urge, 0, expSmoothing(dt, 0.6));
    if (now >= g.activeUntil) return;
    // Let active glance finish naturally; no new decisions.
    return;
  }

  // Smooth jitter so the agent doesn't flip-flop in borderline situations.
  g.jitter = ouStep(g.jitter, dt, 0.9, 0.12);

  const timeSince = Math.max(0, now - g.lastAt);
  const sinceN = clamp01(timeSince / 3.5);

  // Benefit: reduce uncertainty / reacquire.
  let benefit =
    s.uncertainty * 0.95 +
    agent.arousal.pursuitConfidence * 0.65 +
    sinceN * 0.35 +
    (s.threat.aware && !s.threat.visible ? 0.12 : -0.08);

  // Cost: glancing slows you down / is risky when cramped or too close.
  const prox = s.threat.prox;
  const cramped = clamp01((80 - clearanceHere) / 80);
  const sprinting = clamp01((0.8 - sprintT) / 0.8); // early sprint buildup
  let cost =
    (s.threat.visible ? 0.85 : 0) +
    clamp01((prox - 0.55) / 0.25) * 0.75 +
    cramped * 0.7 +
    sprinting * 0.55;

  // Net urge target (0..1).
  const urgeTarget = clamp01(0.55 + benefit - cost);
  g.urge = lerp(g.urge, urgeTarget, expSmoothing(dt, 0.35));

  // If currently glancing, allow early cancellation if we actually reacquire.
  if (now < g.activeUntil) {
    // If we're cramped or the threat is close, stop glancing and focus on moving.
    if (agent.intent.id === "ESCAPE" && (clearanceHere < 55 || prox > 0.62)) {
      g.activeUntil = now;
      g.cooldownUntil = now + randRange(0.35, 0.9);
      setThought(world, agent, `Glance cancelled: danger (clearance=${Math.round(clearanceHere)})`);
      return;
    }

    if (s.threat.visible) {
      const ta = threatAngleForGlance(agent, moveAngle);
      if (Math.abs(angleDiff(agent.heading, ta)) < (14 * Math.PI) / 180) {
        g.activeUntil = now;
        g.cooldownUntil = now + randRange(0.55, 1.4);
        setThought(world, agent, "Glance cancelled: reacquired threat");
      }
    }
    return;
  }
  ...
  // Gate: we don't evaluate every frame as a binary; we only start glances when conditions justify it.
  const majorEvent = s.events.threatLost || s.events.threatProxJump;
  const base =
    agent.intent.id === "ESCAPE"
      ? 0.62
      : 0.75; // foraging glances are rarer

  const threshold =
    clamp01(
      base -
        0.2 * agent.decisionQuality +
        g.jitter * 0.25 +
        (cramped > 0.6 ? 0.18 : 0) +
        (sprinting > 0.4 ? 0.18 : 0) +
        (prox > 0.45 ? 0.12 : 0),
    );

  // If we're trapped near a wall, glancing is too costly: get space first.
  if (agent.intent.id === "ESCAPE" && clearanceHere < 70) {
    if (g.urge > threshold && now - agent.thought.since > 0.5) {
      setThought(world, agent, `Skip glance: need space (clearance=${Math.round(clearanceHere)})`);
    }
    return;
  }

  // Don't glance immediately at escape start: make it "I just started sprinting".
  const timeInIntent = now - agent.intent.enteredAt;
  if (agent.intent.id === "ESCAPE" && timeInIntent < 0.35 && sprinting > 0.55) {
    if (g.urge > threshold) {
      setThought(world, agent, `Skip glance: sprint buildup (${timeInIntent.toFixed(1)}s in)`);
    }
    return;
  }

  const shouldStart = (g.urge > threshold && !s.threat.visible) || (majorEvent && g.urge > 0.45);
  if (!shouldStart) {
    // Only log a skip when urge was meaningful; avoids noisy debug spam.
    if (g.urge > 0.62 && agent.intent.id === "ESCAPE") {
      if (cramped > 0.65) setThought(world, agent, `Skip glance: too close to wall (clearance=${Math.round(clearanceHere)})`);
      else if (prox > 0.55) setThought(world, agent, "Skip glance: too dangerous (too close)");
    }
    return;
  }

  // Start a glance: compute a real target and enough time to actually turn.
  const tune = getArousalTuning(agent.arousal.state);
  const turnRate = tune.turnRate * agent.errorModel.exec.turnScale;
  const targetAngle = threatAngleForGlance(agent, moveAngle);
  const need = Math.abs(angleDiff(agent.heading, targetAngle)) / Math.max(1e-6, turnRate);

  const minDur =
    agent.arousal.state === "PANICKED" ? 0.25 : agent.arousal.state === "ALERT" ? 0.35 : 0.45;
  const maxDur =
    agent.arousal.state === "PANICKED" ? 1.1 : agent.arousal.state === "ALERT" ? 1.4 : 2.4;

  const buffer = 0.10 + (1 - agent.decisionQuality) * 0.12;
  const dur = clamp(need + buffer, minDur, maxDur);

  g.targetAngle = targetAngle;
  g.lastAt = now;
  g.activeUntil = now + dur;
  g.cooldownUntil = g.activeUntil + randRange(0.55, agent.intent.id === "ESCAPE" ? 1.8 : 2.4);

  setThought(
    world,
    agent,
    `Glance: uncertainty=${s.uncertainty.toFixed(2)} pursuit=${agent.arousal.pursuitConfidence.toFixed(2)} clearance=${Math.round(clearanceHere)}`,
  );
}
...
function updateGazePolicy(world, agent, dt, focus, moveAngle) {
  ...
  if (focus === "THREAT") {
    target = threatAngleForGlance(agent, moveAngle);
  ...
  // If actively glancing, override gaze target (this is the "look back" action).
  if (now < agent.gaze.glance.activeUntil) {
    target = agent.gaze.glance.targetAngle;
  }

  // Gentle scanning (smooth, not twitchy) when not locked to a specific target.
  if (focus === "SCAN" && now >= agent.gaze.glance.activeUntil) {
    const amp =
      agent.arousal.state === "CALM"
        ? 0.32
        : agent.arousal.state === "ALERT"
...
function escapeInfluenceFromSpots(agent, candX, candY, dirX, dirY) {
  // Returns { safeBonus, trapPenalty }.
  let safeBonus = 0;
  let trapPenalty = 0;

  for (let i = 0; i < Math.min(6, agent.spatial.safeSpots.length); i++) {
    const sp = agent.spatial.safeSpots[i];
    const dx = sp.x - agent.x;
    const dy = sp.y - agent.y;
    const d = hypot(dx, dy);
    const n = normalize(dx, dy);
    const align = dot(dirX, dirY, n.x, n.y); // same general direction
    if (align < 0.2) continue;
    safeBonus += sp.score * Math.exp(-d / 220) * align * 0.45;
  }

  for (let i = 0; i < Math.min(6, agent.spatial.trapSpots.length); i++) {
    const sp = agent.spatial.trapSpots[i];
    const dx = sp.x - candX;
    const dy = sp.y - candY;
    const d = hypot(dx, dy);
    trapPenalty += sp.score * Math.exp(-d / 180) * 0.55;
  }

  return { safeBonus, trapPenalty };
}

function simulateEscapeCandidate(world, agent, dirAngle, agentSpeed, horizonSec) {
  // Fast short-horizon prediction (no acceleration) to avoid "run into corner then get pinned".
  const r = agent.r;
  const step = 0.12;
  const steps = Math.max(1, Math.round(horizonSec / step));

  let ax = agent.x;
  let ay = agent.y;

  const threatExists = Boolean(world.threat);
  const threatSpeed = threatExists ? world.threat.speed : 0;

  // Pursuer isn't "omniscient" in our model, but the juggernaut is a simple chase bot: it beelines.
  // For planning, we seed the pursuer from what the agent believes / remembers.
  let tx = threatExists ? world.threat.x : 0;
  let ty = threatExists ? world.threat.y : 0;
  if (agent.belief.threat.quality > 0.2) {
    tx = agent.belief.threat.x;
    ty = agent.belief.threat.y;
  } else if (agent.memory.threatPos) {
    tx = agent.memory.threatPos.x;
    ty = agent.memory.threatPos.y;
  }
  ...
  for (let i = 0; i < steps; i++) {
    const nextAx = ax + dirX * agentSpeed * step;
    const nextAy = ay + dirY * agentSpeed * step;
    const cx = clamp(nextAx, r, world.width - r);
    const cy = clamp(nextAy, r, world.height - r);
    if (cx !== nextAx || cy !== nextAy) clampCount += 1;
    ax = cx;
    ay = cy;

    const c = clearance(world, ax, ay);
    minC = Math.min(minC, c);

    if (threatExists && threatSpeed > 0) {
      const toA = normalize(ax - tx, ay - ty);
      tx = clamp(tx + toA.x * threatSpeed * step, world.threat.r, world.width - world.threat.r);
      ty = clamp(ty + toA.y * threatSpeed * step, world.threat.r, world.height - world.threat.r);
      const d = hypot(ax - tx, ay - ty);
      minDist = Math.min(minDist, d);
    }
  }

  const endC = clearance(world, ax, ay);
  const endDist = threatExists ? hypot(ax - tx, ay - ty) : Infinity;

  return { ax, ay, dirX, dirY, minDist, endDist, minC, endC, clampCount };
}

function pickEscapeTarget(world, agent, agentSpeed) {
  // Choose a direction/target that survives the next ~1s and trends toward open space.
  const r = agent.r;
  const threat =
    agent.belief.threat.quality > 0.2
      ? { x: agent.belief.threat.x, y: agent.belief.threat.y }
      : agent.memory.threatPos
        ? agent.memory.threatPos
        : null;

  const awayAngle = threat
    ? angleTo(agent.x - threat.x, agent.y - threat.y)
    : wrapAngle(agent.heading + Math.PI);

  const centerAngle = angleTo(world.width * 0.5 - agent.x, world.height * 0.5 - agent.y);
  const offsetsDeg = [-110, -80, -55, -30, 0, 30, 55, 80, 110];

  const hereC = clearance(world, agent.x, agent.y);
  const preferOpen =
    // Staying away from walls prevents being pinned/cornered.
    hereC < 160 ||
    agent.sensors.threat.prox > 0.22 ||
    agent.arousal.pursuitConfidence > 0.35;
  const minMove = 80;

  const candidates = [];
  // Primary: around "away".
  for (const od of offsetsDeg) candidates.push(awayAngle + (od * Math.PI) / 180);
  // Also consider a "recenter" move (humans do this to avoid being pinned).
  candidates.push(centerAngle);
  candidates.push(lerp(awayAngle, centerAngle, 0.45));
  candidates.push(lerp(awayAngle, centerAngle, 0.65));

  let best = null;
  for (const a of candidates) {
    const sim = simulateEscapeCandidate(world, agent, a, agentSpeed, 1.1);

    const moveD = hypot(sim.ax - agent.x, sim.ay - agent.y);
    if (moveD < minMove) continue; // never "choose" staying in place near a wall

    const c = sim.endC;
    const avgC = avgClearanceAlong(world, agent.x, agent.y, sim.ax, sim.ay, 4);

    // If we're already cramped, only accept moves that open up space.
    if (preferOpen && sim.endC < hereC + 18) continue;

    const distThreat = threat ? hypot(sim.ax - threat.x, sim.ay - threat.y) : 280;
    const cornerPenalty = sim.minC < 60 ? 1 : 0;

    const { safeBonus, trapPenalty } = escapeInfluenceFromSpots(agent, sim.ax, sim.ay, sim.dirX, sim.dirY);

    // Survive-first: penalize predicted close calls.
    const safeDist = 120;
    let closePenalty = 0;
    if (Number.isFinite(sim.minDist) && sim.minDist < safeDist) {
      const d = safeDist - sim.minDist;
      closePenalty = d * d * 0.05;
    }

    // Strongly avoid ending up near walls (this is how you get pinned into corners).
    let wallPenalty = 0;
    if (sim.endC < 100) {
      const d = 100 - sim.endC;
      wallPenalty += d * d * 0.25;
    }
    if (sim.minC < 80) {
      const d = 80 - sim.minC;
      wallPenalty += d * d * 0.35;
    }

    // Preference: open space now + along the path, then distance.
    let score = 0;
    score += distThreat * 0.55;
    score += c * 2.8;
    score += avgC * 1.8;
    score += (sim.minC - r) * 1.6;
    score -= closePenalty;
    score -= wallPenalty;
    score -= cornerPenalty * 520;
    score -= sim.clampCount * 55; // candidates that slam into walls are bad
    score += safeBonus;
    score -= trapPenalty;

    // Bias against massive direction changes unless necessary.
    score -= Math.abs(angleDiff(agent.heading, a)) * 10;

    if (!best || score > best.score) best = { x: sim.ax, y: sim.ay, score, clearance: c };
  }

  if (!best) {
    // Fallback: step toward center (always legal).
    const a = centerAngle;
    const L = 260;
    return {
      x: clamp(agent.x + Math.cos(a) * L, r, world.width - r),
      y: clamp(agent.y + Math.sin(a) * L, r, world.height - r),
      score: 0,
      clearance: clearance(world, agent.x, agent.y),
    };
  }

  // Convert the picked direction into a real target point (ahead of the predicted end).
  const dir = normalize(best.x - agent.x, best.y - agent.y);
  const L = agent.arousal.state === "PANICKED" ? 220 : agent.arousal.state === "ALERT" ? 260 : 300;
  return {
    x: clamp(agent.x + dir.x * L, r, world.width - r),
    y: clamp(agent.y + dir.y * L, r, world.height - r),
    score: best.score,
...
function moveAgent(world, agent, dt, input) {
  ...
  if (agent.mode === "MANUAL") {
    ...
    if (agent.intent.id === "ESCAPE") {
      ...
      // Track escape experience for learning.
      esc.threatSum += agent.sensors.threat.prox;
      esc.threatSamples += 1;

      const hereC = clearance(world, agent.x, agent.y);
      const timeIn = now - agent.intent.enteredAt;

      // Sprint ramp: early escape builds speed/accel (and makes glancing expensive).
      const sprintN = clamp01(timeIn / 0.8);
      const speedRamp = lerp(0.6, 1.0, sprintN);
      const accelRamp = lerp(0.65, 1.0, sprintN);
      agent.errorModel.exec.accelScale *= accelRamp;

      // Mark trap moments (cornered under pressure).
      if (
        agent.sensors.threat.prox > 0.62 &&
        hereC < 70 &&
        now - esc.lastTrapAt > 2.5
      ) {
        const trapScore = (80 - hereC) + agent.sensors.threat.prox * 60;
        upsertSpot(agent.spatial.trapSpots, { x: agent.x, y: agent.y, score: trapScore }, now, 75);
        esc.lastTrapAt = now;
        setThought(world, agent, `Marked trap spot (clearance=${Math.round(hereC)})`);
      }

      // Decide whether to glance (information gathering) vs keep sprinting.
      const moveAngleHint = hypot(agent.vx, agent.vy) > 6 ? angleTo(agent.vx, agent.vy) : agent.heading;
      updateGlanceDecision(world, agent, dt, moveAngleHint, hereC, timeIn);

      // Glancing cost: slows speed + acceleration.
      if (now < agent.gaze.glance.activeUntil) {
        const q = agent.decisionQuality;
        const basePenalty = lerp(0.75, 0.9, q);
        const arPenalty = agent.arousal.state === "PANICKED" ? 0.92 : agent.arousal.state === "ALERT" ? 0.96 : 1.0;
        agent.errorModel.exec.speedScale *= basePenalty * arPenalty;
        agent.errorModel.exec.accelScale *= lerp(0.75, 0.92, q) * arPenalty;
      }

      // Pick an escape target that trends toward open space (commit briefly to avoid jitter).
      if (!esc.target || now >= esc.until) {
        const picked = pickEscapeTarget(world, agent, execSpeed * speedRamp);
        esc.target = { x: picked.x, y: picked.y };
        const crampedN = clamp01((110 - hereC) / 110);
        const baseDur = randRange(0.45, 1.15);
        const arMul = agent.arousal.state === "PANICKED" ? 0.75 : agent.arousal.state === "ALERT" ? 0.95 : 1.05;
        // When cramped, re-evaluate faster to avoid committing to a bad corner line.
        const dur = baseDur * arMul * lerp(0.55, 1.0, 1 - crampedN);
        esc.until = now + dur;
        agent.intent.sub = "RUN_TO_OPEN";
      }

      target = esc.target;
      speed = execSpeed * speedRamp;
      noise = 0.08;
      gazeFocus = "SCAN";
    } else if (agent.intent.id === "FORAGE") {
      gazeFocus = agent.belief.fruit.quality > 0.1 ? "FRUIT" : "SCAN";

      // Optional foraging glances when threat feels uncertain.
      const moveAngleHint = hypot(agent.vx, agent.vy) > 6 ? angleTo(agent.vx, agent.vy) : agent.heading;
      updateGlanceDecision(world, agent, dt, moveAngleHint, clearance(world, agent.x, agent.y), 999);
      if (now < agent.gaze.glance.activeUntil) {
        const q = agent.decisionQuality;
        agent.errorModel.exec.speedScale *= lerp(0.82, 0.92, q);
        agent.errorModel.exec.accelScale *= lerp(0.78, 0.92, q);
      }
...
function drawWorld(world, agent) {
  ...
  if (world.debug && agent.intent.id === "ESCAPE" && agent.intent.escape.target) {
    const t = agent.intent.escape.target;
    ctx.beginPath();
    ctx.arc(t.x, t.y, 7, 0, TAU);
...
function drawHelp(world, agent) {
  ...
  ctx.font = "12px ui-monospace, SFMono-Regular, Menlo, Monaco, Consolas, 'Liberation Mono', 'Courier New', monospace";
  const lines = [
    `v1.2  |  Agent: ${agent.profile.name}  |  mode: ${agent.mode}`,
    "Click: fruit   Shift+Click: threat (juggernaut)",
    "M: manual (WASD)   D: debug   H: help",
    "Watch: deliberate glances + open-area escape + learning + mistakes.",
  ];
  let y = 32;
...
function drawDebug(world, agent) {
  ...
  ctx.strokeStyle = "rgba(0, 0, 0, 0.10)";
  ctx.lineWidth = 1;
  roundedRectPath(ctx, 12, 12, 620, 308, 12);
  ctx.fill();
  ctx.stroke();
  ...
  const g = agent.gaze.glance;
  const glanceLeft = Math.max(0, g.activeUntil - now);
  const cdLeft = Math.max(0, g.cooldownUntil - now);
  const hereC = clearance(world, agent.x, agent.y);
  const esc = agent.intent.escape;
  const escAvgThreat =
    agent.intent.id === "ESCAPE" && esc.threatSamples > 0 ? esc.threatSum / esc.threatSamples : 0;
  const lines = [
    ...
    `glance urge=${g.urge.toFixed(2)}  active=${glanceLeft.toFixed(2)}s  cooldown=${cdLeft.toFixed(2)}s  clearance=${Math.round(hereC)}  safeSpots=${agent.spatial.safeSpots.length} trapSpots=${agent.spatial.trapSpots.length}`,
    `escape avgThreat=${escAvgThreat.toFixed(2)}  thought="${agent.thought.last}"`,
