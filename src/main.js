...
const AGGRO_TACTICS = Object.freeze(new Set(["ATTACK", "PRESSURE", "CLASH"]));
const PLAYER_COMMAND_COOLDOWN = 6.0;
const PLAYER_COMMAND_WINDOW = 6.0;
...
function makeAgent(id, world, x, y, color) {
  return {
    ...
    playerCommand: {
      ...
      issuedAt: -Infinity,
      expiresAt: -Infinity,
      intent: "NONE", // ENGAGE | CHASE | FLANK | RECOVER | REPOSITION
      anchor: null, // {x,y} tap point
      note: "",
      baseForce: 1,
      salvageOnly: false,
      startStamina: 0,
    ...
    nav: {
      ...
      validUntil: 0,
      seed: 0,
      jugOrbitSign: 0, // -1 | +1 side lock while circling the jug
      jugOrbitKey: "",
      jugOrbitUntil: 0,
    },
    avoid: {
...
function jugBlocksRoute(agent, j, target) {
  if (!j || !target) return false;
  const s = agent.senses?.jug;
  const q = s?.quality ?? 0;
  const jugPos = s?.beliefPos ?? (q > 0.6 ? { x: j.x, y: j.y } : null);
  if (!jugPos) return false;

  const keepOut = j.r + agent.r + j.attack.rangePad + 24;
  const d = lineCircleClosestDist(agent.x, agent.y, target.x, target.y, jugPos.x, jugPos.y);
...
function jugAvoidance(world, agent, j, target, dt, opts) {
  ...
  const preferOrbit = Boolean(opts?.preferOrbit);
  const orbitKey = opts?.orbitKey ?? "";
  const now = opts?.now ?? world.time;
  const s = agent.senses?.jug;
  const q = s?.quality ?? 0;
  const jugPos = s?.beliefPos ?? (q > 0.6 ? { x: j.x, y: j.y } : null);
  ...
  const sideDelta = cR - cL;
  let sign = sideDelta > 10 ? -baseSign : baseSign;
  const prevOrbitSign = agent.nav?.jugOrbitSign ?? 0;
  const prevOrbitKey = agent.nav?.jugOrbitKey ?? "";
  const prevOrbitUntil = agent.nav?.jugOrbitUntil ?? -Infinity;
  const keepLockedSide =
    preferOrbit &&
    prevOrbitSign !== 0 &&
    prevOrbitKey === orbitKey &&
    now <= prevOrbitUntil &&
    Math.abs(sideDelta) < 26;
  if (keepLockedSide) sign = prevOrbitSign;

  if (preferOrbit && orbitKey) {
    agent.nav.jugOrbitSign = sign;
    agent.nav.jugOrbitKey = orbitKey;
    agent.nav.jugOrbitUntil = now + 0.8;
  } else if (!preferOrbit && now > prevOrbitUntil) {
    agent.nav.jugOrbitSign = 0;
    agent.nav.jugOrbitKey = "";
  }
  ...
  const centerDist = hypot(agent.x - jugPos.x, agent.y - jugPos.y);
  const keepOut = j.r + agent.r + j.attack.rangePad + 10;
  const closeToSkin = clamp01((keepOut + 36 - centerDist) / 36);
  ...
  const tangMul = preferOrbit
    ...
    : preferRadial
      ...
      : wrapAllowed
        ? 1.0
        : 0.35;
  const awayMul = preferOrbit
    ? 0.22 + 0.88 * closeToSkin
    : preferRadial
      ? 1.15
      : wrapAllowed
        ? 0.55
  ...
  const costPerSec = (9 + 18 * intensity) * (0.55 + 0.45 * awareMul) * (preferRadial ? 0.8 : preferOrbit ? 1.05 : 1.0);
  return { vx, vy, intensity, costPerSec };
}

function jugOrbitWaypoint(world, agent, j, target, opts = null) {
  // Deterministic detour waypoint used when the jug blocks the route.
  if (!j || !target) return null;
  const now = opts?.now ?? world.time;
  const escapeMode = Boolean(opts?.escapeMode);
  const force = Boolean(opts?.force);
  const orbitKey = opts?.orbitKey ?? "";

  const s = agent.senses?.jug;
  const q = s?.quality ?? 0;
  const jugPos = s?.beliefPos ?? (q > 0.25 ? { x: j.x, y: j.y } : { x: j.x, y: j.y });

  const keepOut = j.r + agent.r + j.attack.rangePad + (escapeMode ? 34 : 24);
  const lineD = lineCircleClosestDist(agent.x, agent.y, target.x, target.y, jugPos.x, jugPos.y);
  const blocked = lineD < keepOut;
  if (!blocked && !force) return null;

  const angA = Math.atan2(agent.y - jugPos.y, agent.x - jugPos.x);
  const angT = Math.atan2(target.y - jugPos.y, target.x - jugPos.x);
  const diff = angleDiff(angA, angT); // target angle from current angle around the jug
  const absDiff = Math.abs(diff);

  const prevSide = agent.nav?.jugOrbitSign ?? 0;
  const prevKey = agent.nav?.jugOrbitKey ?? "";
  const prevUntil = agent.nav?.jugOrbitUntil ?? -Infinity;
  let side = 0;
  if (prevSide !== 0 && prevKey === orbitKey && now <= prevUntil && absDiff > 0.08) side = prevSide;
  if (!side) side = diff >= 0 ? 1 : -1;
  if (!side) side = ((agent.nav.seed >>> 1) & 1) ? 1 : -1;

  const stepMin = escapeMode ? 0.90 : 0.62;
  const stepMax = escapeMode ? 1.35 : 1.08;
  let step = clamp(absDiff * 0.75 + (escapeMode ? 0.35 : 0.18), stepMin, stepMax);
  if (escapeMode && force) step = Math.max(step, 1.1);

  const angW = angA + side * step;
  const orbitR = keepOut + (escapeMode ? 84 : 64);
  const tang = { x: -Math.sin(angW) * side, y: Math.cos(angW) * side };
  const lead = escapeMode ? 88 : 62;
  const x = clamp(jugPos.x + Math.cos(angW) * orbitR + tang.x * lead, agent.r, world.width - agent.r);
  const y = clamp(jugPos.y + Math.sin(angW) * orbitR + tang.y * lead, agent.r, world.height - agent.r);

  const clearNow = lineD > keepOut * 1.08;
  const closeTarget = hypot(agent.x - target.x, agent.y - target.y) < 220;
  ...
  return {
    ...
    side,
    orbitKey,
    exitReady: clearNow && absDiff < 0.22 && closeTarget,
  };
}
...
function inferPlayerTapIntent(world, agent, opp, j, tap) {
  const hpN = clamp01(agent.hp / Math.max(1e-6, agent.maxHp));
  const oppHpN = clamp01(opp.hp / Math.max(1e-6, opp.maxHp));
  const finishP = estimateFinishProb(agent, opp);
  const selfLow = hpN < 0.28 || agent.stamina < 22;
  const oppLow = oppHpN < 0.36 || finishP > 0.36;

  const dTapSelf = hypot(tap.x - agent.x, tap.y - agent.y);
  const dTapOpp = hypot(tap.x - opp.x, tap.y - opp.y);
  const dTapJug = j ? hypot(tap.x - j.x, tap.y - j.y) : Infinity;
  const nearOpp = dTapOpp < opp.r + 78;
  const nearSelf = dTapSelf < agent.r + 84;
  const nearJug = j ? dTapJug < j.r + 90 : false;
  const jugBetween =
    j &&
    lineCircleClosestDist(agent.x, agent.y, opp.x, opp.y, j.x, j.y) < j.r + agent.r + 18;

  let intent = "REPOSITION";
  let note = "set up better lane";
  if (nearOpp) {
    if (oppLow || oppHpN + 0.08 < hpN || !selfLow) {
      intent = "ENGAGE";
      note = "commit on enemy now";
    } else {
      intent = (nearJug || jugBetween) ? "FLANK" : "CHASE";
      note = "pressure but route around danger";
    }
  } else if (nearJug) {
    if (oppLow) {
      intent = "FLANK";
      note = "attack route around jug";
    } else if (selfLow) {
      intent = "RECOVER";
      note = "reposition to safer lane";
    } else {
      intent = "REPOSITION";
      note = "shift line near jug";
    }
  } else if (nearSelf && selfLow) {
    intent = "RECOVER";
    note = "stabilize and recover";
  } else if (dTapOpp < 180 && (oppLow || !selfLow)) {
    intent = "CHASE";
    note = "close distance and force exchange";
  } else if (selfLow && dTapOpp > 200) {
    intent = "RECOVER";
    note = "play safe and recover";
  } else if (dTapOpp < 320) {
    intent = "REPOSITION";
    note = "improve attack angle";
  }

  let baseForce = 1.12;
  if (intent === "ENGAGE") baseForce = 1.30;
  else if (intent === "CHASE") baseForce = 1.22;
  else if (intent === "FLANK") baseForce = 1.20;
  else if (intent === "RECOVER") baseForce = 1.16;
  if (selfLow && !oppLow && (intent === "ENGAGE" || intent === "CHASE")) baseForce -= 0.12;

  return {
    intent,
    note,
    anchor: { x: tap.x, y: tap.y },
    baseForce: clamp(baseForce, 0.85, 1.4),
    salvageOnly: false,
  };
}

function issuePlayerCommand(world, agent, opp, j, tap) {
  const now = world.time;
  ...
  const cmd = agent.playerCommand ?? {};
  const cmdId = (cmd.id ?? 0) + 1;
  agent.playerCommand = {
    active: true,
    id: cmdId,
    issuedAt: now,
    expiresAt: now + PLAYER_COMMAND_WINDOW,
    intent: inferred.intent,
    anchor: inferred.anchor,
    note: inferred.note,
    baseForce: inferred.baseForce,
    salvageOnly: inferred.salvageOnly,
    startStamina: agent.stamina,
    goalSince: -Infinity,
  ...
  setThought(
    agent,
    `PLAYER ${inferred.intent} (${Math.round(tap.x)},${Math.round(tap.y)}) | ${inferred.note}`,
    now,
  );
  return { ok: true, intent: inferred.intent, wait: 0 };
}
...
function resolvePlayerDirective(world, agent, opp, j, ctx = null) {
  ...
  const safeDistEff = ctx?.safeDistEff ?? ((j ? desiredSafeDistToJug(agent) : 260) * lerp(0.55, 1.0, j ? agent.senses.jug.quality : 0));
  ...
  const imminentJug =
    ...
    Boolean(
      j &&
      ((now - agent.events.jugWindupSeenAt < 0.4 && dJ < jugHitRange + 34) ||
      (agent.senses.jug.visible && j.windupUntil > now && j.windupTargetId === agent.id && dJ < jugHitRange + 46)),
    );

  const aggressiveIntent = commandIntentIsAggressive(cmd.intent);
  const hitRange = agent.r + opp.r + MELEE_REACH;
  const dealtHitSinceCmd = (agent.events?.dealtHitAt ?? -Infinity) > cmd.issuedAt;
  const exchangeNow =
    dO < hitRange * 1.18 &&
    (AGGRO_TACTICS.has(agent.tactic) ||
      (agent.attackWindupUntil ?? 0) > now ||
      (opp.attackWindupUntil ?? 0) > now ||
      now - (agent.events?.gotHitAt ?? -Infinity) < 0.25 ||
      now - (opp.events?.gotHitAt ?? -Infinity) < 0.25);
  const distToAnchor = hypot(agent.x - cmd.anchor.x, agent.y - cmd.anchor.y);

  if (aggressiveIntent) {
    if (dealtHitSinceCmd || exchangeNow) {
      if (!Number.isFinite(cmd.goalSince)) cmd.goalSince = now;
      if (now - cmd.goalSince > 0.2) {
        cmd.active = false;
        cmd.completedAt = now;
        return null;
      }
    } else {
      cmd.goalSince = -Infinity;
    }
  } else if (cmd.intent === "REPOSITION") {
    if (distToAnchor < 18) {
      if (!Number.isFinite(cmd.goalSince)) cmd.goalSince = now;
      if (now - cmd.goalSince > 0.25 && hypot(agent.vx, agent.vy) < 52) {
        cmd.active = false;
        cmd.completedAt = now;
        return null;
      }
  ...
  } else if (cmd.intent === "RECOVER") {
    const stamN = clamp01((agent.stamina ?? 0) / Math.max(1e-6, agent.maxStamina));
    const staminaGain = (agent.stamina ?? 0) - (cmd.startStamina ?? (agent.stamina ?? 0));
    const oppFarEnough = dO > hitRange * 2.15 || opp.hp <= 0;
    const jugSafe = !j || dJ > safeDistEff * 1.24;
    const recoveredEnough = stamN > 0.58 || staminaGain > 18 || (hpN < 0.35 && stamN > 0.68);
    const heldLongEnough = now - cmd.issuedAt > 1.0;
    if (jugSafe && oppFarEnough && recoveredEnough && heldLongEnough) {
      if (!Number.isFinite(cmd.goalSince)) cmd.goalSince = now;
      if (now - cmd.goalSince > 0.55) {
        cmd.active = false;
        cmd.completedAt = now;
        return null;
      }
  ...
  const jugPinned = Boolean(j && dJ < safeDistEff * 0.78);
  const lowHpCrisis = hpN < 0.12;
  let impossible = 0;
  if (imminentJug) impossible += 0.74;
  if (jugPinned) impossible += 0.45;
  if (lowHpCrisis && aggressiveIntent) impossible += 0.35;
  impossible = clamp01(impossible);
  const impossibleHigh = aggressiveIntent ? impossible > 0.86 : impossible > 0.92;
  if (impossibleHigh) {
    if (!Number.isFinite(cmd.blockedSince)) cmd.blockedSince = now;
  } else {
    cmd.blockedSince = -Infinity;
  }
  const blockedLongEnough = Number.isFinite(cmd.blockedSince) && now - cmd.blockedSince > 0.45;
  const salvage = cmd.salvageOnly || blockedLongEnough;

  const force = salvage
    ? clamp(cmd.baseForce * 0.62, 0.55, 0.9)
    : clamp(cmd.baseForce * (1.26 - impossible * 0.12), 1.02, 1.65);

  const objective = { recover: 0, duel: 0, bait: 0 };
  const tactic = makeTacticBiasMap();
  switch (cmd.intent) {
    case "ENGAGE":
      objective.recover -= 1.35; objective.duel += 2.55; objective.bait += 0.45;
      tactic.ATTACK += 560; tactic.PRESSURE += 420; tactic.CLASH += 240;
      tactic.RETREAT_SHORT -= 120; tactic.OPEN_UP -= 300; tactic.RETREAT_LONG -= 400; tactic.RESET -= 260;
      break;
    case "CHASE":
      objective.recover -= 1.0; objective.duel += 2.0; objective.bait += 0.8;
      tactic.PRESSURE += 510; tactic.ATTACK += 330; tactic.CLASH += 150;
      tactic.RESET -= 220; tactic.OPEN_UP -= 230; tactic.RETREAT_LONG -= 320; tactic.RETREAT_SHORT -= 130;
      break;
    case "FLANK":
      objective.recover -= 0.72; objective.duel += 1.38; objective.bait += 1.65;
      tactic.PRESSURE += 350; tactic.ATTACK += 220; tactic.RETREAT_SHORT += 85; tactic.RESET += 65;
      tactic.OPEN_UP -= 180; tactic.RETREAT_LONG -= 250;
      break;
    case "RECOVER":
      objective.recover += 2.6; objective.duel -= 1.5; objective.bait -= 0.3;
      tactic.RETREAT_LONG += 300; tactic.OPEN_UP += 240; tactic.RESET += 180; tactic.RETREAT_SHORT += 110;
      tactic.ATTACK -= 190; tactic.PRESSURE -= 140; tactic.CLASH -= 160;
      break;
    default:
      objective.recover += 0.42; objective.duel += 0.92; objective.bait += 0.42;
      tactic.RESET += 540; tactic.OPEN_UP += 300; tactic.RETREAT_SHORT += 220; tactic.RETREAT_LONG += 130;
      tactic.PRESSURE -= 70; tactic.ATTACK -= 260; tactic.CLASH -= 150; tactic.BLOCK -= 140;
      break;
  }

  if (salvage) {
    objective.recover += 1.7;
    objective.duel += aggressiveIntent ? 0.7 : 0.25;
    objective.bait += aggressiveIntent ? 0.45 : 0.2;
    tactic.ATTACK -= 260;
    tactic.PRESSURE += 110;
    tactic.RESET += 90;
    tactic.RETREAT_SHORT += 90;
    tactic.OPEN_UP += 60;
    tactic.RETREAT_LONG += 50;
  }

  const pursueOpp = aggressiveIntent;
...
function computeObjectiveWeights(world, agent, opp, j, ctx = null) {
  ...
  if (playerDirective?.active) {
    const force = playerDirective.force ?? 1;
    recoverScore += (playerDirective.objective?.recover ?? 0) * force;
    duelScore += (playerDirective.objective?.duel ?? 0) * force;
    baitScore += (playerDirective.objective?.bait ?? 0) * force;
  }
...
function planGoalForTactic(world, agent, opp, j, tactic, ctx, weights, rng) {
  ...
  if (playerDirective?.active && playerDirective.anchor) {
    pushCand(playerDirective.anchor.x, playerDirective.anchor.y, "PLAYER_ANCHOR");
  }
  ...
  function scoreCand(cand) {
    ...
    if (cand.kind === "PLAYER_ANCHOR") score += 260;
    const goalKindBias = ctx?.mbtiBias?.goalKind?.[cand.kind] ?? 0;
    score += goalKindBias;
    if (playerDirective?.active && playerDirective.anchor) {
      const force = playerDirective.force ?? 1;
      const dTap = hypot(cand.x - playerDirective.anchor.x, cand.y - playerDirective.anchor.y);
      const tapPull = clamp(260 - dTap, -180, 260);
      score += tapPull * (playerDirective.salvage ? 0.7 : 0.95) * force;

      const meleeReach = MELEE_REACH;
      const hitRange = agent.r + opp.r + meleeReach;
      if (playerDirective.pursueOpp) {
        const dOppGoal = hypot(cand.x - opp.x, cand.y - opp.y);
        const desiredOppDist =
          playerDirective.intent === "CHASE"
            ? hitRange * 1.85
            : playerDirective.intent === "FLANK"
              ? hitRange * 1.55
              : hitRange * 1.08;
        score -= Math.abs(dOppGoal - desiredOppDist) * (playerDirective.salvage ? 0.58 : 0.85) * force;
        if (cand.kind === "DUEL_RING" || cand.kind === "ATTACK_LANE") score += 165 * force;
        if (playerDirective.intent === "FLANK" && cand.kind === "BAIT_LINE") score += 120 * force;
      } else if (playerDirective.intent === "RECOVER") {
        if (cand.kind === "ESCAPE_LANE" || cand.kind === "SAFE_MEMORY") score += 170 * force;
        if (cand.kind === "DUEL_RING") score -= 110 * force;
      } else if (playerDirective.intent === "REPOSITION" && cand.kind === "CENTER") {
        score += 90 * force;
      }

      if (playerDirective.salvage && cand.kind === "ESCAPE_LANE") score += 130 * force;
      if (playerDirective.intent === "REPOSITION" && !playerDirective.salvage) {
        score -= dTap * 2.8 * force;
        if (cand.kind === "PLAYER_ANCHOR") score += 520 * force;
      }
    }
...
function tacticTarget(world, agent, opp, j, tactic, rng) {
  ...
  if (playerDirective?.active && playerDirective.intent === "REPOSITION" && !playerDirective.salvage && playerDirective.anchor) {
    return { x: playerDirective.anchor.x, y: playerDirective.anchor.y };
  }
...
function decideTactic(world, agent, opp, j) {
  ...
  for (const id of TACTICS) {
    ...
    if (scene === "RESET") {
      ...
      if ((id === "ATTACK" || id === "CLASH") && !playerAggroIntent && !oppSeen && !recentEngage) continue;
      if (id === "PRESSURE" && !playerAggroIntent && !oppSeen && !recentEngage && !jugFarForEngage) continue;
    }
    ...
    if (playerAggroIntent && !imminentJug) {
      const allowed =
        id === "ATTACK" ||
        id === "PRESSURE" ||
        id === "CLASH" ||
    ...
    if (playerRepositionIntent && !imminentJug) {
      const allowed = id === "RESET" || id === "OPEN_UP" || id === "RETREAT_SHORT" || id === "RETREAT_LONG";
      if (!allowed) continue;
    }
    if (playerRecoverIntent) {
      const allowed = id === "RETREAT_LONG" || id === "OPEN_UP" || id === "RESET" || id === "RETREAT_SHORT" || id === "BLOCK";
      if (!allowed) continue;
    }
    ...
    if (playerDirective?.active) score += (playerDirective.tactic?.[id] ?? 0) * (playerDirective.force ?? 1);
    if (playerAggroIntent && !imminentJug) {
      const force = playerDirective.force ?? 1;
      if (id === "ATTACK") score += 540 * force;
      else if (id === "PRESSURE") score += 460 * force;
      else if (id === "CLASH" && dO < hitRange * 2.2) score += 290 * force;
      else if (id === "RETREAT_SHORT") score -= 90 * force;
      else if (id === "BLOCK" && !oppImminent) score -= 170 * force;
    }
    if (playerRepositionIntent) {
      const force = playerDirective.force ?? 1;
      if (id === "RESET") score += 360 * force;
      else if (id === "OPEN_UP") score += 280 * force;
      else if (id === "RETREAT_SHORT") score += 220 * force;
      else if (id === "RETREAT_LONG") score += 140 * force;
    }
    if (playerRecoverIntent) {
      const force = playerDirective.force ?? 1;
      if (id === "RETREAT_LONG") score += 560 * force;
      else if (id === "OPEN_UP") score += 460 * force;
      else if (id === "RESET") score += 320 * force;
      else if (id === "RETREAT_SHORT") score += 220 * force;
      else if (id === "BLOCK" && oppImminent) score += 180 * force;
      else if (id === "ATTACK" || id === "PRESSURE" || id === "CLASH") score -= 520 * force;
    }
  ...
  setThought(
    ...
    `Pick ${chosen.id} (${agent.posture}) | dJ=${Math.round(dJ)} dO=${Math.round(dO)} ` +
      ...
      (playerDirective?.active ? ` | player=${playerDirective.intent}${playerDirective.salvage ? ":salvage" : ""}` : "") +
...
function executeTactic(world, agent, opp, j, dt) {
  ...
  let windupActive = agent.attackWindupUntil > now && agent.attackWindupTargetId === opp.id;
  const garrisonActive = stanceIsActive(agent, "GARRISON", now);
  const assaultActive = stanceIsActive(agent, "ASSAULT", now);
  const style = agent.style ?? {};
  ...
  const playerDirective = resolvePlayerDirective(world, agent, opp, j, {
    ...
    imminentJug: Boolean(
      j &&
        ((now - agent.events.jugWindupSeenAt < 0.4 && dJ < (agent.r + j.r + j.attack.rangePad) + 34) ||
          (agent.senses.jug.visible && j.windupUntil > now && j.windupTargetId === agent.id && dJ < (agent.r + j.r + j.attack.rangePad) + 46)),
    ),
  });
  const playerCommandActive = Boolean(playerDirective?.active && !playerDirective.salvage);
  const playerAggroCommand = Boolean(playerCommandActive && commandIntentIsAggressive(playerDirective.intent));
  const playerRepositionCommand = Boolean(playerCommandActive && playerDirective.intent === "REPOSITION");
  const playerRecoverCommand = Boolean(playerCommandActive && playerDirective.intent === "RECOVER");
  if (playerRecoverCommand && windupActive) {
  ...
  if (playerRepositionCommand && playerDirective.anchor) {
    tgt = { x: playerDirective.anchor.x, y: playerDirective.anchor.y };
    dir = normalize(tgt.x - agent.x, tgt.y - agent.y);
    agent.nav.target = tgt;
    agent.nav.validUntil = now + 0.3;
    speed *= threatenedNow ? 0.95 : 1.08;
  }
  ...
  const routeToTarget = j ? jugRouteRisk(world, agent, j, tgt) : { risk: 0, detourCost: 0 };
  const jugBlocksTargetExact = Boolean(
    j &&
      lineCircleClosestDist(agent.x, agent.y, tgt.x, tgt.y, j.x, j.y) <
        (j.r + agent.r + j.attack.rangePad + 26),
  );
  const toJugNow = j ? normalize(j.x - agent.x, j.y - agent.y) : { x: 0, y: 0, len: 0 };
  const toTargetNow = normalize(tgt.x - agent.x, tgt.y - agent.y);
  const targetTowardJug = Boolean(
    j &&
      toJugNow.len > 0 &&
      toTargetNow.len > 0 &&
  ...
  const jugBlocksTarget = Boolean(
    j &&
      (
        (routeToTarget.risk > 0.24 && jugBlocksRoute(agent, j, tgt)) ||
        (playerRepositionCommand && jugBlocksTargetExact) ||
        retreatFacingJug
      ),
  );
  const preferOrbitAroundJug = Boolean(
    jugBlocksTarget &&
      (playerRepositionCommand || playerAggroCommand || (playerCommandActive && !playerRecoverCommand)),
  );
  const commandNeedsPathing = playerRepositionCommand || playerAggroCommand || playerRecoverCommand;
  const needDetourArc = Boolean(
    j &&
      (
  ...
  const orbitKey = playerCommandActive
    ? `cmd:${agent.playerCommand?.id ?? 0}`
    : `tac:${agent.tactic}|${Math.round(tgt.x)}:${Math.round(tgt.y)}`;

  if (needDetourArc) {
    const detour = jugOrbitWaypoint(world, agent, j, tgt, {
      now,
      force: retreatFacingJug || (playerRepositionCommand && jugBlocksTargetExact),
      escapeMode: retreating || playerRecoverCommand,
      orbitKey,
    });
    if (detour) {
      tgt = { x: detour.x, y: detour.y };
      agent.nav.target = tgt;
      agent.nav.validUntil = now + (retreating || playerRecoverCommand ? 0.36 : 0.28);
      agent.nav.jugOrbitSign = detour.side;
  ...
  if (doingCloseWork || agent.tactic === "RESET" || threatened || commandNeedsPathing) {
    const preferRadial = Boolean(
      !(preferOrbitAroundJug || retreatFacingJug) &&
        (
          threatenedNow ||
    ...
    jugAvoidV = jugAvoidance(world, agent, j, tgt, dt, {
      preferRadial,
      wrapAllowed: wrapAllowedNow && !preferRadial && !(preferOrbitAroundJug || retreatFacingJug),
      preferOrbit: preferOrbitAroundJug || retreatFacingJug,
      orbitKey,
      now,
    });
  }
  ...
  if (j) {
    ...
    if (dJReal < bumpPad) {
      ...
      const side = preferOrbitAroundJug
        ? (agent.nav.jugOrbitSign || (((agent.nav.seed >>> 1) & 1) ? 1 : -1))
        : (((agent.nav.seed >>> 1) & 1) ? 1 : -1);
      const tang = { x: -away.y * side, y: away.x * side };
      const awayMag = preferOrbitAroundJug
        ? (120 + 190 * bumpI) * (threatenedNow ? 1.0 : 0.85)
        : (180 + 280 * bumpI) * (threatenedNow ? 1.0 : 0.85);
      const tangMag = preferOrbitAroundJug ? 140 + 210 * bumpI : 55 + 95 * bumpI;
      jugBumpV = {
  ...
  if (dir.len < 34 && (objectives.recover ?? 0) > 0.55 && !threatenedNow && !playerCommandActive) {
    speed *= 0.65;
  ...
  const wantsAttack =
    ...
    (((agent.tactic === "ATTACK" || agent.tactic === "PRESSURE" || agent.tactic === "CLASH") && now >= agent.feintUntil) ||
      opportunisticJab);
  const forcedAttack = playerAggroCommand && !threatenedNow && dO <= hitRange * 2.8;
    ...
    if (agA && agB && j) {
      ...
      function shouldInterrupt(agent) {
        ...
        if (commandLocked) {
          ...
          if (agent.events.pinnedSince > ld && world.time - agent.events.pinnedSince > 0.38) return true;
          return false;
        }
        if (agent.events.gotHitAt > ld) return true;
        if (agent.events.tookBigHitAt > ld) return true;
